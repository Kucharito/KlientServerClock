...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        if(strncmp(l_buf,"READY\n",6)==0){
            write(STDOUT_FILENO,l_buf,l_len);
            break;
        }
    }

    FILE *file=fopen(l_filename,"r");
    if(!file)
    {
    ...
    char l_buf[256]={};
    int l_len;    
    while((l_len=fread(l_buf,sizeof(char),sizeof(l_buf),file))>0){
        write(l_sock_server,l_buf,l_len);
    }
