...
void handle_client(int socket)
{
    ...
    write(socket, "READY\n", 6);
    FILE *file = fopen("clock.cpp", "w");
    if (!file)
    {
        log_msg(LOG_INFO, "COuld not open a file");
    }
    char l_char[1]{};
    while ((l_len = read(socket, l_char, 1)) > 0)
    {
        if (strncmp(l_buf, "\n", 1) == 0)
        {
            l_row_count++;
        ...
        fwrite(l_char, sizeof(char), 1, file);
        fflush(file);
        l_buf[0] = '\0';
    }
    pid_t pid = fork();
    if (pid == 0)
    {
        execlp("g++", "g++", "clock.cpp", "-o", "clock", (char *)nullptr);
    }
    else
    {
        int status;
        ...
        if ((WEXITSTATUS(status)) == 0)
        {
            ...
            if (pid2 == 0)
            {
                ...
                execl("./clock", "./clock", (char *)nullptr);
